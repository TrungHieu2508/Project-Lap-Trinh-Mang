...
class ServerMonitorGUI:
    ...
    def update_computers_display(self):
        """Cập nhật danh sách máy tính kết nối - hàng đều, max 3 khung/1 hàng"""
        ...
        # Tính chiều cao tối đa của frame (bao gồm top_info + img_area)
        frame_width = 350  # chiều rộng cố định 
        img_height = int(frame_width * 3 / 4)  # tỉ lệ 4:3
        top_info_height = 30  # chiều cao cố định cho top_info
        total_frame_height = top_info_height + img_height + 20  # padding
        ...
        for cid, info in self.connected_clients.items():
            ...
            tk.Label(top_info, text=name_text, font=('Arial', 14, 'bold'), bg='#ecf0f1', fg='#2c3e50').pack(side='left', anchor='w')
            ...
            tk.Label(top_info, text=status_text, font=('Arial',12,'bold'), fg=status_color,bg='#ecf0f1').pack(side='right', anchor='e')
            ...
            # Hiển thị ảnh giữ tỉ lệ 4:3
            def display_image(frame_area, image_data):
                for widget in frame_area.winfo_children():
                    widget.destroy()
                if image_data:
                    try:
                        ...
                        lbl = tk.Label(frame_area, image=photo, bg='black')
                        lbl.image = photo
                        lbl.pack(expand=True, fill='both')
                    except:
                        tk.Label(frame_area, text="Lỗi ảnh", bg='black', fg='red').pack(expand=True)
                else:
                    tk.Label(frame_area, text="Chưa có hình ảnh", bg='black', fg='white', font=('Arial', 12)).pack(expand=True)

            display_image(img_area, info['image_data'])
